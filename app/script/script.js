...
window.onload = function () {
  const useLibraryURLs = localStorage.getItem('wayakuExtensions')
    ? librariesURL.concat(JSON.parse(localStorage.getItem('wayakuExtensions')))
    : librariesURL;
  ...
  const setStart = setInterval(function () {
    ...
    if (finishedScriptNumber != resentScriptNumber) {
      console.log(
        `${useLibraryURLs[finishedScriptNumber - 1]}の読み込み時間:約${
          scriptSetLoopNumber / 1000
        }秒`
      );
